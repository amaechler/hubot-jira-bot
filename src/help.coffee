...
class Help
  ...
  @forTopic: (topic, robot) ->
    ...
    *assigned* to a ticket
    ...
    creating = """
    *Creating Tickets*
    > #{robot.name} jira create [`<project>`] `<type>` `<title>` [`<description>`]
    ...
    *Searching Tickets*
    > #{robot.name} jira search `<term>`
        *Optional `<term>` Attributes*
            _Projects_: search in a specific project
                 `#ap`

    Where `<term>` is some text contained in the ticket you are looking for
    """

    if _(["create", "report", "open", "file", "subtask", _(Config.maps.types).keys()]).chain().flatten().contains(topic).value()
      responses = [ creating ]
